...
class ModelCharacteristics:
    ...
    def __init__(self, hard_criteria: HardCriteria, soft_criteria: SoftCriteria, version: int = 0,
                 identity: str = None):
    ...
    def _get_weighted_score(cls, model, hard_criteria, weights):
        if model.hard_criteria != hard_criteria:
            return float('inf')
        return 1 - model.soft_criteria['accuracy'] * weights['accuracy'] \
...
if __name__ == '__main__':
    ...
    sorted = sorted(models, key=ModelCharacteristics.generate_key_function(
        hard_criteria={"model_type": "regression", "input_type": "tabular", "output_format": "audio"},
        weights={"energy_efficiency": 1, "accuracy": 1, "latency": 1, "latency_range": {"min": 20, "max": 120},
                 "cached": 1}))
